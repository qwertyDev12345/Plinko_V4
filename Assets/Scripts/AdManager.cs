...
public class AdManager : MonoBehaviour
{
    ...
    [SerializeField] private float firstInterstitialDelaySeconds = 60f;
    [SerializeField] private int firstInterstitialBallsRequired = 0;
    ...
    private void ShowBanner()
    {
        ...
        Debug.Log(
            $"[ADMANAGER] Calling ShowBannerBottom " +
            $"Init={zeyWin?.IsInitialized} " +
            $"ProviderReady={zeyWin?.IsBannerReady}"
        );
    ...
    private void ShowBannerForced(AdNetwork net)
    {
        ...
        // ZeyWinAds legacy sticky native banner
        _currentBannerNet = AdNetwork.ZeyWin;
        HideBanner(AdNetwork.AdMob);
        zeyWin?.ShowBannerBottom();
    }
