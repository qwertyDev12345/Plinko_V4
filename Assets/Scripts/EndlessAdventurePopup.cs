...
public class EndlessAdventurePopup : Popup
{
    ...
    private void OnClickCardHandler(EndlessAdventureCard card)
    {
        ...
        Debug.Log("OnClickCardHandler");
        Debug.Log($"_busy = {_busy}");
        Debug.Log($"_activeRewardPanel = {_activeRewardPanel}");
        Debug.Log($"cardsCount = {_endlessAdventureCards.Count}");
        Debug.Log($"clicked == current ? {card == _endlessAdventureCards[0]}");
        ...
        Debug.Log($"card.IsFree = {card.IsFree}");
        ...
        else
        {
            ...
            {
                StartCoroutine(ShowRewardDelayed(card));
            });
    ...
    private IEnumerator ShowRewardDelayed(EndlessAdventureCard card)
    {
    ...
    {
        Debug.Log("ShowRewarded ENTER");

        AdManager adManager = FindFirstObjectByType<AdManager>();

        Debug.Log($"AdManager found = {adManager != null}");
        ...
        {
            Debug.Log("Rewarded Success Callback");
